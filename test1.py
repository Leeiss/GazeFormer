...
def test(args):
    ...
    for target_traj in tqdm(test_task_img_pairs):
        ...
        for idx, scanpath in enumerate(scanpaths):
            print(f"\n[✓] Predicted Scanpath #{idx + 1} for {task_name}_{name}_{condition}:")
            for fix in scanpath:
                y, x, t = fix  # Порядок: [y, x, t], как в `run_model`
...
def main(args):
    ...
    print('Sequence Score : {:.3f}, Sequence Score with Duration : {:.3f}, Semantic Sequence Score : {:.3f}'.format(
        seq_score, seq_score_t, sem_seq_score))
